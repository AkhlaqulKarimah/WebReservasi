<?php
	...
	$tanggal = date('Ymd');
	$query = "SELECT max(kode_reservasi) as maxkode FROM tbl_reservasi WHERE date('Ymd') LIKE '$tanggal%'";
	$hasil = mysqli_query($con,$query);

	if (isset($hasil)) {
		$kode = mysqli_query($con,$hasil);
		$ambilKode = mysqli_fetch_array($kode);
		...
		$kode = mysqli_query($con, "SELECT max(kode_reservasi) as maxkode FROM tbl_reservasi");
		$ambilKode = mysqli_fetch_array($kode);
		$kodereservasi = $ambilKode['maxkode'];
